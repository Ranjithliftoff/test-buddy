...
def get_or_create_session(db: Session, module: str, source_type: Optional[str]=None, source_url: Optional[str]=None, notes: Optional[str]=None) -> m.Session:
    q = db.query(m.Session).filter(m.Session.module==module, m.Session.status=="active")
    if source_type:
        q = q.filter(m.Session.source_type==source_type)
    s = q.first()
    if s:
        return s
...
def add_scenarios(db: Session, session_id, titles: List[str]) -> List[m.Scenario]:
    items = [m.Scenario(session_id=session_id, title=t) for t in titles]
